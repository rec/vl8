...
class TestBoundFunction(unittest.TestCase):
    def test_a_simple_function(self):
        f = BoundFunction("test.old.function.test_function.simp(required: true)")
        assert f.missing == set()
        assert tuple(f("one")) == (("one", True, 1),)

    def test_simple_function2(self):
        f = BoundFunction("test.old.function.test_function.simp(r: true)")
        assert f.missing == set()
        assert tuple(f("one")) == (("one", True, 1),)

    def test_simple_error(self):
        f = BoundFunction("test.old.function.test_function.simp")
